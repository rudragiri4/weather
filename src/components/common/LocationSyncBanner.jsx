...
export const LocationSyncBanner = () => {
  ...
  return (
    <div className="bg-gradient-to-r from-slate-900/90 via-[#0e1628]/90 to-slate-900/90 border border-slate-800 rounded-3xl p-4 sm:p-5 shadow-xl backdrop-blur-xl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 text-xs font-semibold transition"
          >
            <Search className="w-3.5 h-3.5" />
            <span>{showSearch ? 'Close Search' : 'Change City'}</span>
          </button>
        </div>
      </div>
      ...
      {/* Expanded Search Bar */}
      {showSearch && (
        <div className="mt-4 pt-4 border-t border-slate-800/80 animate-slideUp">
          <SearchBar placeholder="Search any city or coordinates worldwide..." />
        </div>
      )}
    </div>
  );
